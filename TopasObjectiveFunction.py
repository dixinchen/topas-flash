...
def CalculateObjectiveFunction(film1_dose_file, film2_dose_file, temp_topas_pdd_file):

    film1_dose = np.zeros([binx, biny, binz])
    data_dose = LoadData(film1_dose_file)
    ...
    data_dose = LoadData(film2_dose_file)
    for j in range(len(data_dose)):
    ...
    ## load TOPAS PDD
    temp_topas_pdd = LoadData(temp_topas_pdd_file)
    ...
    # total mse
    # ObjectiveFunction = mse_top_x + mse_bot_x + mse_top_y + mse_bot_y + mse_pdd + 0.2
    ObjectiveFunction = mse_pdd

    return ObjectiveFunction  # only difference from the geometry optimisation is that we will take the log here!


def TopasObjectiveFunction(ResultsLocation, iteration):

    film1_dose_file = str(ResultsLocation) + '/DoseAtFilm1_itt_' + str(iteration) + '.csv'
    film2_dose_file = str(ResultsLocation) + '/DoseAtFilm2_itt_' + str(iteration) + '.csv'
    temp_topas_pdd_file = str(ResultsLocation) + '/DoseAtWaterTank_itt_' + str(iteration) + '.csv'

    OF = CalculateObjectiveFunction(film1_dose_file, film2_dose_file, temp_topas_pdd_file)
    return OF